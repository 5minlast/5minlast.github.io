...
async function fetchFacilities() {
    const res = await fetch(`/api/facilities?sido=${state.currentSido}`);
    state.data.facilities = await res.json();
    document.getElementById('fac-count').innerText = `${state.data.facilities.length}개`;
}

async function fetchMetrics() {
    const res = await fetch(`/api/metrics?sido=${state.currentSido}`);
...
async function updateMapForSido(sido) {
    ...
    const res = await fetch(`/api/coords?sido=${sido}`);
